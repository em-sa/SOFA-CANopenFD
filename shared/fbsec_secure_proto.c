...
/* Signed-FBsec adds a 64-byte Ed25519 trailer after the tag. */
#define SIGNED_RESPONSE_BUF_LEN ((uint16_t)(RESPONSE_BUF_LEN + FBSEC_ASYM_SIG_SIZE))
#define SIGNED_FRAMED_BUF_LEN   ((uint16_t)(FRAMED_BUF_LEN + FBSEC_ASYM_SIG_SIZE))
#endif
...
#if FBSEC_ASYM_SIGNED_FBSEC
/* ---- Signed-FBsec single-shot (spec 11.6.5) --------------------------- */
  ...
  uint32_t *len_out, fbsec_abort_t *abort_out)
{
  ...
  uint8_t skid = (uint8_t)(key_id | 0x20u);   /* signed-FBsec flag (bit 5) */
  ...
  fbsec_secure_status_t rc = transport->write(transport->ctx, device_id, data_id,
                                              challenge, CHALLENGE_LEN_RD,
                                              timeout_ms, abort_out);
  if (rc != FBSEC_SECP_OK) return rc;

  uint8_t  response[SIGNED_RESPONSE_BUF_LEN];
  uint32_t resp_len = 0u;
  rc = transport->read(transport->ctx, device_id, data_id, NULL, 0u,
                       response, sizeof response, timeout_ms, &resp_len, abort_out);
  if (rc != FBSEC_SECP_OK) return rc;

  const uint16_t header_len = (uint16_t)FBSEC_AEAD_RAND_SIZE;
  const uint16_t overhead   = (uint16_t)(header_len + FBSEC_AEAD_TAG_SIZE + FBSEC_ASYM_SIG_SIZE);
  if (resp_len < overhead) return FBSEC_SECP_PROTOCOL;
  uint16_t data_len = (uint16_t)(resp_len - overhead);
  ...
  /* AEAD tag first (spec 11.6.5: cheap check gates the costly verify). */
  if (!fbsec_aead_open(key, nonce, FBSEC_AEAD_DIR_READ_RESPONSE, skid,
                       device_id, fbsec_secure_port_get_client_id(), data_id,
                       client_random, server_random,
                       &response[header_len], data_len,
                       &response[header_len + data_len], buf)) {
    return FBSEC_SECP_TAG;
  }

  /* Then verify the server's Ed25519 signature over the transcript. */
  const uint8_t *sig = &response[header_len + data_len + FBSEC_AEAD_TAG_SIZE];
  fbsec_pubkey_t pk;
  uint8_t  ctx[FBSEC_ASYM_FBSEC_CTX_LEN];
  uint8_t  transcript[FBSEC_ASYM_TRANSCRIPT_OVERHEAD + FBSEC_ASYM_FBSEC_CTX_LEN];
  uint16_t clen = fbsec_asym_fbsec_context(data_id, device_id,
                                           fbsec_secure_port_get_client_id(),
                                           client_random, server_random,
  ...
  if (!fbsec_secure_port_peer_pubkey(device_id, pk.pub) ||
      (clen == 0u) || (tlen == 0u) ||
  ...
  const uint8_t *buf, uint32_t len, uint32_t timeout_ms, fbsec_abort_t *abort_out)
{
  ...
  /* Pass 1: fetch server_random. */
  uint8_t  server_random[FBSEC_AEAD_RAND_SIZE];
  uint32_t server_random_len = 0u;
  fbsec_secure_status_t rc = transport->read(transport->ctx, device_id, data_id,
                                             NULL, 0u, server_random, sizeof server_random,
                                             timeout_ms, &server_random_len, abort_out);
  if (rc != FBSEC_SECP_OK) return rc;
  ...
  /* Pass 2: skid || client_random || cipher || tag || SIG[64]. */
  uint8_t  framed[SIGNED_FRAMED_BUF_LEN];
  framed[0] = skid;
  memcpy(&framed[1], client_random, FBSEC_AEAD_RAND_SIZE);
  uint8_t *write_cipher = &framed[1u + FBSEC_AEAD_RAND_SIZE];
  uint8_t *write_tag    = &framed[1u + FBSEC_AEAD_RAND_SIZE + len];
  if (!fbsec_aead_seal(key, nonce, FBSEC_AEAD_DIR_WRITE_REQUEST, skid,
                       device_id, fbsec_secure_port_get_client_id(), data_id,
                       client_random, server_random, buf, (uint16_t)len,
                       write_cipher, write_tag)) {
    return FBSEC_SECP_TAG;
  }

  uint8_t *sig_out = &framed[1u + FBSEC_AEAD_RAND_SIZE + len + FBSEC_AEAD_TAG_SIZE];
  uint8_t  ctx[FBSEC_ASYM_FBSEC_CTX_LEN];
  uint8_t  transcript[FBSEC_ASYM_TRANSCRIPT_OVERHEAD + FBSEC_ASYM_FBSEC_CTX_LEN];
  uint16_t clen = fbsec_asym_fbsec_context(data_id, device_id,
                                           fbsec_secure_port_get_client_id(),
  ...
  uint16_t tlen = fbsec_asym_transcript(FBSEC_ASYM_RD_SIGNED_FBSEC_C2S, ctx, clen,
                                        transcript, (uint16_t)sizeof transcript);
  if ((clen == 0u) || (tlen == 0u) ||
      !fbsec_secure_port_sign(FBSEC_ASYM_RD_SIGNED_FBSEC_C2S, transcript, tlen, sig_out)) {
    return FBSEC_SECP_TAG;
  }

  return transport->write(transport->ctx, device_id, data_id, framed,
                          (uint32_t)(1u + FBSEC_AEAD_RAND_SIZE + len
                                     + FBSEC_AEAD_TAG_SIZE + FBSEC_ASYM_SIG_SIZE),
                          timeout_ms, abort_out);
}
#endif /* FBSEC_FEATURE_WRITE */
